...
const Sidebar = () => {
  ...
  return (
    <div className="h-full w-[250px] border-r bg-zinc-800 flex flex-col overflow-y-auto overflow-x-hidden mb-2">
      {/* HEADER */}
      ...
      <div className="p-4 text-md flex flex-col h-full justify-between gap-2">
        ...
        <div className="flex flex-col gap-3 mt-4">
          <Divider />
          <div className="flex flex-col gap-2">
            <h2 className="text-white font-medium">SSH Hosts</h2>
            <HostManager />
          </div>
          ...
          {hosts.length > 0 && (
            <div className="text-xs text-gray-400 mt-1">
              {activeHost ? (
                <p>Active host: {hosts.find(h => h.id === activeHost)?.username}@{hosts.find(h => h.id === activeHost)?.host}</p>
              ) : (
                <p>No active host selected</p>
              )}
            </div>
          )}
        </div>
